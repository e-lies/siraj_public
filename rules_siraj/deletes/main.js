...
$('body').ready(function(e){
  ...
  $(".selected").change(function(e){
    $(this).parents().eq(1).find('.form-control').prop('disabled', !$(this).is(':checked'));
  });

  // Checking all selectables
  $("#columns_table .selected_all").change(function(e){
    $(".selected").prop('checked', $(this).is(':checked'));
    $("#columns_table .form-control").prop('disabled', !$(this).is(':checked'));
  });
...
$("input.col_label").change(function(e){
  ...
  $(`select.cond_label option[data-col-name="${$(e.target).data('col-name')}"]`).replaceWith(
    $(`<option  value="${$(e.target).data('col-name')}" data-col-name="${$(e.target).data('col-name')}">${$(e.target).val()}</option>`)
  );
});
      ...
      function(data){
        ...
        switch (auth_type) {
          ...
          case 'varchar':
            ...
            break;
          case 'int':
            auth_var_values = int_from_obj("form-control auth_var_val", auth_var_name);
            break;
          case 'float':
            auth_var_values = float_from_obj("form-control auth_var_val", auth_var_name);
            break;
          default:
...
function cond_row(labels){
  ...
  labels = labels_select_from_obj(labels, 'form-control cond_label', 'Select column');
  var options = [
    {label:'<' ,value:'<'},
    {label:'>' ,value:'>'},
    {label:'<>' ,value:'<>'},
