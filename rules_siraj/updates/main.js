...
$('body').ready(function(e){
  ...
  $("#columns_table .selected_all").change(function(e){
    ...
    $("#columns_table .form-control:not(.path)").prop('disabled', !$(this).is(':checked'));
  });
  ...
  $("button.f_remove").click(function(e){
    var col_name = $(this).parents().eq(3).find('.f_col').val();

    if(!confirm('Are you sure ?')){
      return;
    }
    $(this).parents().eq(3).fadeOut(400, function(e){
      var row = $(`#columns_table tr[data-col-name='${col_name}']`);
      $(row).find('.type_link').remove();
      $(row).find('.col_type').replaceWith(`<input ${ (( !$(row).find('.selected').is(':checked') ) ? 'disabled' : '') } type="text" class="form-control col_type" value=${$(row).find('.old_type').val()} />`);
      $(this).remove();
    })
  });

  // Adding a new extention
  $("button#add_cond").click(function(e){

    var labels = [];

    var labels = [];
      $('.table#columns_table .col_label').each( function(index, input){
        labels.push({
          label:$(input).data("col-name"),
          value:$(input).data("col-name"),
          col_name:$(input).data("col-name"),
          col_type:$(input).data('col-type'),
    ...
    $('#cond_wrapper').append(cond_row(labels));
  });
...
// $("input.col_label").change(function(e){
//   // return console.log(  $(`select.ext_label option[data-col-name="${$(e.target).data('col-name')}"]`));
//   // $("select.ext_label option").prop('selected', false);
//   var elem = $(`select.cond_label option[data-col-name="${$(e.target).data('col-name')}"]`);
//   $(elem).replaceWith(
//     $(`<option  value="${$(e.target).data('col-name')}" data-col-name="${$(e.target).data('col-name')}">${$(e.target).val()}</option>`)
//   );
// });
...
function cond_row(labels){
  ...
  labels = labels_select_from_obj(labels, 'form-control cond_col', 'Select column');
  var options = [
    {label:'<' ,value:'<'},
    {label:'>' ,value:'>'},
    {label:'<>' ,value:'<>'},
  ...
  var ext_row = $(`
    <div class="row cond_row">
      ...
      <div class="col-lg-2">
        <input type="text" placeholder="Label" class="form-control cond_label" />
      </div>
      <div class="col-lg-2">
        <input type="text" placeholder="type" class="form-control cond_type" />
      </div>
      <div class="col-lg-2">
        ${select_from_obj(options, 'form-control cond_oper', 'Choose operator')}
      </div>
      <div class="col-lg-2">
        <label for="">Required</label>
        <input class="cond_required" type="checkbox" />
      </div>
      <div class="col-lg-2">
...
$("body").ready(function(e){
  $("body").delegate(".cond_col", "change", function(e){
    var parents = $(this).parents().eq(1);
    $(parents).find('.cond_label').val($(this).val());

    $(parents).find('.cond_type').val($(this).find('option:selected').data('col-type'));
