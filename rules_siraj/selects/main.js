...
$('body').ready(function(e){
  ...
  $("button#add_ext").click(function(e){
    $("#ext_wrapper .card-body").append(ext_row());
  });
...
// Changing the operands and extention value in case the label type on the extention was changes
  $("body").delegate('select.ext_label', 'change', function(e){
    ...
    switch (type.data_type) {
      case 'set':
        ...
        ext_values = checkboxes_from_obj(possibilities, "ext_value", label);
        break;
    ...
    // return console.log(ext_opers);
    $(row).find('.ext_value').replaceWith(ext_values);
    $(row).find('.ext_oper').replaceWith(ext_opers);
  });


  // changing the operands and values in case the var name changes
...
function labels_select_from_obj(object, class_name, placeholder){
  options = ``;
  object.forEach(function(element){
    options += `<option data-col-name="${element.col_name}" value="${element.value}">${element.label}</option>`;
  });
