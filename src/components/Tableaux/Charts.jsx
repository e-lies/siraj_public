...
import { Typography, Select, MenuItem, IconButton, Icon } from '@material-ui/core';
import {
  clientPies,
  clientLines,
  clientBMR,
...
const Charts = (props) => {
  const {
    title, type, schema, data, chartParams, chartProps, onChange, saveReport
  } = props;
  ...
  useEffect(()=>{
    ...
    if(schema && schema.columns && data && data.length > 0){
      let sch = Object.keys(data[0]).reduce((acc,cur)=>{
  ...
  const pieUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.label && obj.value) {  
      var dt = newSchema.columns[obj.label]['type'].includes('foreign')
        ? data.map(d=>{
          return {...d,
            [obj.label]:newSchema.columns[obj.label].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.label])}) ? newSchema.columns[obj.label].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.label])})['label'] : d[obj.label]
            }
          })
        : data
  ...
  const radarUpdate = (obj) => {
    setForm(obj)
    if (newSchema && newSchema.columns && obj.group && obj.index && obj.Y && obj.op) {
      var dt = newSchema.columns[obj.group]['type'].includes('foreign')
        ? data.map(d=>{
          return {...d,
            [obj.group]:newSchema.columns[obj.group].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.group])}) ? newSchema.columns[obj.group].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.group])})['label'] : d[obj.group]
            }
          })
        : data
  ...
  const matrixUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.group && obj.index && obj.Y && obj.op) {
      var dt = newSchema.columns[obj.index]['type'].includes('foreign')
        ? data.map(d=>{
          return {...d,
            [obj.index]:newSchema.columns[obj.index].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.index])}) ? newSchema.columns[obj.index].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.index])})['label'] : d[obj.index]
            }
          })
        : data
      const formattedData = clientBMR(dt, obj.group, obj.index, obj.Y, obj.op);
  ...
  const barUpdate = (obj) => {
    setForm(obj)
    if (newSchema && newSchema.columns && obj.X !== null && obj.Y !== null && obj.op !== null) {
      var dt = newSchema.columns[obj.index]['type'].includes('foreign') 
        ? data.map(d=>{
          return {...d,
            [obj.index]:newSchema.columns[obj.index].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.index])}) ? newSchema.columns[obj.index].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.index])})['label'] : d[obj.index]
  ...
  const lineUpdate = (obj) => {
    ...
    if (newSchema && newSchema.columns && obj.X && obj.Y && obj.op) {
      var dt = newSchema && newSchema.columns && newSchema.columns[obj.id]['type'].includes('foreign')
      ? data.map(d=>{
        return {...d,
          [obj.id]:newSchema.columns[obj.id].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.id])}) ? newSchema.columns[obj.id].possibles.find(p=>{return parseInt(p.value)===parseInt(d[obj.id])})['label'] : d[obj.id]
          }
        })
      : data
      if(newSchema && newSchema.columns && newSchema.columns[obj.X] && newSchema.columns[obj.X].type==="time"){
        dt = dt.map(d=>{return {...d,[obj.X]:"1970-01-01T"+d[obj.X]} })
      }
      const formattedData = clientLines(dt, obj.id || null, obj.X, obj.Y, obj.op, obj.color, obj.precision, newSchema && newSchema.columns[obj.X] ? newSchema.columns[obj.X].type : 'date');
      let xLabel = newSchema && newSchema.columns[obj.X] ? newSchema.columns[obj.X].label : obj.X
      let yLabel = newSchema && newSchema.columns[obj.Y] ? newSchema.columns[obj.Y].label : obj.Y
  ...
  if(newSchema && newSchema.columns){
    return (
      <div>
        {!chartParams && (<div style={{display:'flex',justifyContent:'space-around'}}>
         <Select
          ...
          { saveReport &&
           <IconButton color="secondary" onClick={()=>saveReport(chartType,form)}>
